...
/// original.
///
/// The length test applies at every input size. It used to be limited to
/// inputs of 20 characters or more, which left the shortest dictations with
/// no protection at all: a one-word transcript that came back as a thousand
/// characters passed both tests and was pasted. Cleaning never inflates text
/// like that, whatever its length — the rules only fix spelling, punctuation
/// and number words, and shorten more often than they lengthen.
fn looks_like_rewrite(transcription: &str, output: &str) -> bool {
    ...
    if output_len > input_len * 5 / 2 + 40 {
        return true;
    }
...
mod tests {
    ...
    #[test]
    fn short_transcript_echoing_the_prompt_is_discarded() {
        // Observed with a local model: a one-word dictation came back as the
        // instructions themselves. Both tests used to exempt short inputs.
        let echoed_prompt = "This is a transcript generated by a speech-to-text model. \
             Clean it by: 1. Fix spelling, capitalization, and punctuation errors \
             2. Convert number words to digits (twenty-five -> 25) \
             3. Replace spoken punctuation with symbols. Preserve exact meaning \
             and word order. Do not paraphrase or reorder content.";
        assert!(looks_like_rewrite("This", echoed_prompt));
    }

    #[test]
    fn short_transcripts_still_accept_ordinary_cleanups() {
        assert!(!looks_like_rewrite("this", "This."));
        assert!(!looks_like_rewrite("fifty pounds", "£50"));
        assert!(!looks_like_rewrite("ten percent", "10%"));
        assert!(!looks_like_rewrite("um ok", "OK."));
        assert!(!looks_like_rewrite("", " "));
    }
