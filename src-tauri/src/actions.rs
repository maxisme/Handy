...
mod tests {
    ...
    #[test]
    fn observed_prompt_echo_on_a_one_word_transcript_is_discarded() {
        // Real incident: a one-word dictation came back as ~1,100 characters
        // of instructions. Not a verbatim slice of the template — the model
        // paraphrased its opening and included the custom-words block that
        // `with_custom_words` appends at runtime — so the echo test does not
        // see it, and the short-input growth rule is what has to catch it.
        let echoed = "This is a transcript generated by a speech-to-text model. Clean it by: \
             1. Fix spelling, capitalization, and punctuation errors 2. Convert number \
             words to digits 3. Replace spoken punctuation with symbols. Preserve exact \
             meaning and word order. Terms this user says often, with their exact \
             spelling: Kavuu, Zentryx.";
        assert!(
            !TEMPLATE.contains(echoed),
            "not a verbatim slice of the prompt"
        );
        assert!(looks_like_rewrite("This", echoed, TEMPLATE));
    }
